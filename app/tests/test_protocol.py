...
ON_EXAMPLE = bytes.fromhex("00 04 80 00 00 0d 0e 0b 3b 23 00 00 00 00 00 00 00 32 00 00 90")
...
def test_on_frame_matches_reverse_example() -> None:
    frame = build_on_frame(seq=0x04)
    assert frame == ON_EXAMPLE
    assert len(frame) == 21
    assert frame[2:5] == bytes.fromhex("80 00 00")
    assert frame[7:10] == bytes.fromhex("0B 3B 23")
...
def test_hsv_red_uses_0b_3b_a1() -> None:
    frame = build_hsv_frame(255, 0, 0, seq=0x05)
    assert len(frame) == 21
    assert frame[7:10] == bytes.fromhex("0B 3B A1")
    assert rgb_to_lednetwf_hsv(255, 0, 0) == (0, 100, 100)
    assert frame[10:13] == bytes([0, 100, 100])


def test_color_kind_dispatch() -> None:
    rgb = build_color_frame(0, 255, 0, kind="rgb", seq=1)
    hsv = build_color_frame(0, 255, 0, kind="hsv", seq=1)
    assert rgb[8] == 0x31
    assert hsv[8:10] == bytes.fromhex("3B A1")
    assert rgb != hsv
